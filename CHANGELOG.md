...
- **New GA image model `gemini-3.1-flash-lite-image`** ("Nano Banana 2 Lite", GA 2026-06-30) added to `models.json` and surfaced by `tiny-gemini models` — the fastest/cheapest Gemini image model (1K only, 10 standard aspect ratios). It is the recommended cheapest-image successor to the deprecated `gemini-2.5-flash-image` (shutdown 2026-10-02)
- **`gemini-2.5-pro-preview-tts`** added to the `models.json` registry (Audio, Preview, $1.00/$20.00, no free tier) — a selectable 2.5-family TTS model alongside `gemini-2.5-flash-preview-tts`; `gemini-3.1-flash-tts-preview` remains the recommended default. Verbatim-verified against the live docs 2026-07-16
